...
rbp.db=read.delim("../data/rbp.target.binary.txt",header=T,as.is=T,sep="\t")
...
for(i in 1 : length(targetIDs)){
 ...
 cv <- cross_validation(K=K,R=R,X0=X0,X1=X1,Y=Y,trim=F)
 if(is.list(cv)==TRUE) {
...
write.table(RCV,file="../results/rcv.summary.txt",quote=F,row.names=F,
 col.names=RCVcolnames,sep="\t")
